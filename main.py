...
def run(train_df, valid_df, test_df, meta_features, n_features, train_tfs, valid_tfs, mel_idx, loss_fn, seed, threshold = 0.005, vis_path = "learning_curves", dl_save_path = "saved_dls"):
    ...
    torch.save(test_dl, f"./{dl_save_path}/{args.save_name}_test_dl.pth")
    ...
    auc_max, not_improve, patience = 0., 0, 3
    best_ckpt_file  = f"{args.model_dir}/{args.save_name}_best.pth"
    last_ckpt_file  = f"{args.model_dir}/{args.save_name}_last.pth"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss = train_epoch(model, train_dl, optimizer, use_feats = args.use_meta, device = args.device, image_size = args.image_size, loss_fn = loss_fn)
        val_loss, acc, auc = valid_epoch(model, valid_dl, mel_idx, use_feats = args.use_meta, device = args.device, n_cls = args.n_cls, loss_fn = loss_fn)
        ...
        plot_learning_curves(plot_data = losses, title = "Losses", save_name = f"{args.save_name}_loss", save_path = vis_path, loss_data = True)
        plot_learning_curves(plot_data = accs, title = "Accuracy Score", save_name = f"{args.save_name}_acc", save_path = vis_path)
        plot_learning_curves(plot_data = aucs, title = "AUC Score", save_name = f"{args.save_name}_auc", save_path = vis_path)
...
if __name__ == '__main__':
    ...
    run(train_df = train_df, valid_df = valid_df, test_df = test_df, meta_features = meta_features, n_features = n_features, train_tfs = train_tfs, valid_tfs = valid_tfs, mel_idx = mel_idx, loss_fn = loss_fn, seed = seed)